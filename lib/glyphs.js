...
module.exports = function (style, outDir, accessToken, cb) {
  if (!style.glyphs) return cb(new Error('no glyph url defined in style'))
  console.log('Downloading glyphs..')
  mkdirp(outDir)
  var fontStacks = getFontStacks(style)
  downloadGlyphs(style.glyphs, fontStacks, outDir, accessToken, cb)
}
...
function downloadGlyphs (urlTemplate, fontStacks, outDir, accessToken, cb) {
  ...
  fontStacks.forEach(stack => {
    ...
    for (var i = 0; i < 65536; (i = i + 256)) {
      range = i + '-' + (i + 255)
      url = glyphUrl(stack, range, urlTemplate)
